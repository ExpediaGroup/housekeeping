...
## Password Encryption
Housekeeping allows users to provide encrypted passwords in their configuration or programs. The Housekeeping project depends on the [jasypt library](http://www.jasypt.org/download.html) which can be used to generate encrypted passwords which in turn can be decrypted by Spring Boot's jasypt support.
