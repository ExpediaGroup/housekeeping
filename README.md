...
Applications which leverage housekeeping support can define their own schema and table which they desire their Housekeeping data to be stored into.
This can be acheived in three steps.

Firstly, the user must create their database initialisation schema.sql script, and add it to their classpath (or alternatively provide it as a resource in your application).
...
Within your application you will need to extend two base classes provided in the Housekeeping project: `LegacyReplicaPath` and `LegacyReplicaPathRepository`.
These classes provide skeleton implementations of the LegacyReplicaPath `Entity` and the LegacyReplicaPathRepository `CrudRepository`.
...
## Password Encryption
Housekeeping allows users to provide encrypted passwords in their configuration or programs. The Housekeeping project has the jasypt-1.9.2.jar in the /lib directory. This jar can be used to generate encrypted passwords which can be decrypted by Spring Boots jasypt support in your application. The necessary dependencies for decrypting jasypt encrypted passwords are packaged with the Housekeeping project.
...
Finally if you are using a encrypted password, when you run your application you must provide the application with your
jasypt.encryptor.password there are a few approaches to doing so.
