...
public class TunnelConnectionManagerFactory {
  ...
  private static int getLocalPort() {
    try (ServerSocket socket = new ServerSocket(FIRST_AVAILABLE_PORT)) {
      ...
      throw new RuntimeException("Unable to bind to a free localhost port", e);
    }
  }
  ...
  public TunnelConnectionManager create(
      String route,
      String localHost,
      int localPort,
      String remoteHost,
    ...
    try {
      ...
      throw new RuntimeException("Unable to create a TunnelConnectionManager: " + tunnelExpression, e);
    } finally {
